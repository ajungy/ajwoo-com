 ...
 * import from inside a component), and `...rest` added so this project's
 * own `data-cursor-label` (read by components/Cursor.tsx) can reach the
 * underlying `<button>`. The WebGL shader logic itself — the rounded-rect
 * signed-distance field, the pointer-steered light angle, the render
 * loop — is untouched from the registry source.
 *
 ...
 *     per button instance via a dedicated <canvas> — further from "plain
 *     CSS" than anything else on this site.
 ...
 *     no user action driving it. Same objection that got the
 *     `javascript-snow` script cut in Phase 0 and that required
 *     ParticleText (this session, on the landing headline) to be built
 *     as an explicit, logged exception before Alex reverted it shortly
 *     after.
 * Alex asked for this exact component, with this exact prop
 * configuration, after being shown both conflicts directly and
 * confirming the deviation is intentional — same treatment ParticleText
 * got. It's wired into ShareButton.tsx only; nothing else on the site
 * uses WebGL.
 */
