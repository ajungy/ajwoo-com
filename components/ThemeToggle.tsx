 ...
 *
 * The flip itself is a plain 300ms crossfade, not a color sweep — an earlier
 * version played a multi-stage white→orange→navy→black animation; Alex asked
 * for that removed in favor of a simple fade back to the system's actual
 * white/black surfaces. `data-theme-transition` is set on `<html>` for
 * exactly that 300ms, which fades every foreground AND background color
 * (text, icons, card surfaces, the page background itself — see the
 * `[data-theme-transition]` rule in globals.css) at the same pace instead of
 * snapping instantly.
 */
const FADE_MS = 300;
